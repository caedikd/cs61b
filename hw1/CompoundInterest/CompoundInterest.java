...
public class CompoundInterest {
    ...
    static double futureValueReal(double presentValue, double rate, int targetYear, double inflationRate) {
        ...
        return futureVal * Math.pow(((100-inflationRate)/100), numYears(targetYear));
    }
    ...
    static double totalSavings(double perYear, int targetYear, double rate) {
        ...
        while (numYears(targetYear) >= 0) {
            total += perYear * Math.pow(((100+rate)/100), numYears(targetYear));
            targetYear -= 1;
        }
        return total;
    }
    ...
    static double totalSavingsReal(double perYear, int targetYear, double rate, double inflationRate) {
        //should be able to do this in 2 lines?
        double initialtot = totalSavings(perYear, targetYear, rate);
        return initialtot * Math.pow(((100 - inflationRate)/100), numYears(targetYear));
    }
    ...
    static void printSavingsFV(int targetYear, double returnRate,
        ...
        double nominalSavings = futureValue(PER_YEAR, returnRate, targetYear); // replace 0 with your code
        double realSavings = futureValueReal(PER_YEAR, returnRate, targetYear, inflationRate);    // replace 0 with your code
